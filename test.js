...
function Mission1(unit) {
 let rank  = unit.commandStructure.chiefOfStaff.rank;
 let name  = unit.commandStructure.chiefOfStaff.name;
  let  phone = unit.commandStructure.chiefOfStaff.contact.phone;
  answer = rank+ " "+name+" "+ phone;
return answer;
}
...
{
    let count =unit.personnel.length;
...
function Mission4(unit, newvehicl)
{
    ...
    newquantity =newvehicl.quantity;
    newtype = newvehicl.type;
    newstatus = newvehicl.status

      let found = false;
      unit.equipment.vehicles.forEach((vehicl, index) => {
          if (vehicl.type === newtype  && vehicl.type === newtype) {
              unit.equipment.vehicles[index].quantity +=newquantity;
              found = true;  
          }
      });  
      if (!found) {
          unit.equipment.vehicles.push(newvehicl);
      }
...
{
    let count =0;
    unit.trainingPrograms.forEach((trainingProgram, index) => {
         count+=trainingProgram.duration;
    });  
    return count;   
}
module.exports = {
